...
async def test_missing_token_raises(deps, monkeypatch):
    ...
    bv = BuildVerifyOutcome(built=True, matched=True, visual_attempts_used=0, build_attempts_used=0)
...
async def test_idempotent_site_reuse(deps, monkeypatch):
    ...
    def fake_runner(d, bv, token):
        ...
        return DeployOutcome(deployed=True, url="https://x.netlify.app", site_id="site-123")
